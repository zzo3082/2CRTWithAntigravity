...
## 10. 效能優化與進階狀態精簡：1D 陣列 + 三進位 (Base-3) 卷積演算法

在原先的直觀實作中，精確計算解析解（`compute_aoi_periodic.m` 與 `compute_aoi_gatw.m`）時，為追蹤各個使用者的碰撞分佈組合，採用 `containers.Map` 以及字串組合（如 `'0120...'`）來作為狀態的 Key。然而，當使用者數量 $K \ge 13$ 時，迴圈中每次狀態展開都會面臨巨大的效能瓶頸（狀態空間呈指數暴增，且 MATLAB 中字串操作與雜湊表讀寫極度緩慢），導致單一使用者運算時間從幾秒鐘飆升至數小時。

為此，在目前的實作中導入了**「1D 陣列 + 三進位 (Base-3) 索引」**的高效演算法，大幅解決了效能卡死的問題。

### 為什麼這套解法是最有效率的？

1. **狀態降維 (Capping at 2) 封鎖狀態空間爆炸：**
   根據公式定義，不論是 Periodic 還是 Generate-at-will traffic，最終只關心一個 slot 的碰撞總數是否 `≤ 1`（成功）或是 `≥ 2`（失敗）。因此可以將所有的碰撞累加限制在最高 2 次（`min(hits, 2)`）。這使得長度為 K 的狀態，每個元素嚴格限制在 `{0, 1, 2}` 範圍內，總狀態空間被**完美的理論上限鎖定在 $3^K$ 以內**。

2. **零字串處理，全面矩陣向量化 (Vectorization \& Matrix Ops)：**
   徹底拋棄字元相加與 ASCII 轉換，將每一個狀態直接視作一個三進位 (Base-3) 的整數，並透過一個純數值的 1D 陣列 `P_old`（大小為 $3^K$）來儲存組合數。例如撞擊分佈為 `[c1, c2, c3, ...]` 的狀態，其 Array Index 等於 `1 + c1*3^0 + c2*3^1 + c3*3^2 + ...`。配合 `repmat` 等矩陣運算，可以在不使用慢速 for-loop 的情況下快速解碼狀態。

3. **捨棄慢速雜湊表 (Hash Map) 改用 Array / accumarray：**
   MATLAB 存取數值 Array 陣列的速度是讀取 `containers.Map`（Hash 表）的數百乃至數千倍。將狀態映射為純整數 index 後，可以直接使用 MATLAB 最優化的 C 語言底層函式 `accumarray` 來一次性完成相同狀態機率的合併加總（Convolution）。

**總結：**
這套解法在保留**100% 絕對解析精確度**（非蒙地卡羅近似）的前提下，完全捨棄了效能最差的字串操作與動態 Map 新增，不僅將記憶體控制在固定的小範圍內，也使大 K 值（如 K=13）的運算速度提升了成千上萬倍，從數小時縮短至非常合理的時間。
